...
const challenge = {
 init: (elevators, floors) => {
  ...
  const go = (elevator, n) => {
   ...
   elevator.destinationQueue = [...new Set(elevator.destinationQueue)].sort((a, b) => (
    elevator.goingUpIndicator() ? a - b : b - a
   ));
  ...
  const serve = (elevator, dir, comp, sorter) => {
   const filteredFloors = floors
    .filter(({ level }) => comp(level, elevator.currentFloor()) && floorButton(level) === dir)
    .sort(sorter);
