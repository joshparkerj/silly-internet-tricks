...
(function worldometerCovidTable() {
 ...
 fetch(document.URL)
  ...
  .then((chartOptions) => {
   ...
   const datesString = chartOptions[0].slice(
    chartOptions[0].indexOf('[', xaxisCategoriesPos),
    1 + chartOptions[0].indexOf(']', xaxisCategoriesPos),
   );
   ...
   const bodyRows = dates
    .map(
     (date, i) => `<tr title="${days[new Date(date).getDay()]}"><td>${date}</td>${dataColumns
      .map(({ data }) => `<td>${data[i]}</td>`)
      .join('')}</tr>`,
    )
    .join('');
   ...
   document.styleSheets[0].insertRule(
    '#covid-table { margin: 2rem 6rem; border: solid black; font-size: small; }',
   );
   document.styleSheets[0].insertRule(
    '#covid-table th, #covid-table td { border: solid 0.2px darkgray; }',
   );
   document.styleSheets[0].insertRule(
    '#covid-table th { position: sticky; top: -1px; background: lightblue; padding: 0.4rem 1rem; }',
   );
   document.styleSheets[0].insertRule(
    '#covid-table td { padding: 0.3rem 1.5rem; text-align: right; }',
   );
   document.styleSheets[0].insertRule(
    '#covid-table tr[title=sunday] { background-color: hsl(300, 100%, 85%); }',
   );
   document.styleSheets[0].insertRule(
    '#covid-table tr[title=monday] { background-color: hsl(280, 100%, 87%); }',
   );
   document.styleSheets[0].insertRule(
    '#covid-table tr[title=tuesday] { background-color: hsl(260, 100%, 89%); }',
   );
   document.styleSheets[0].insertRule(
    '#covid-table tr[title=wednesday] { background-color: hsl(240, 100%, 91%); }',
   );
   document.styleSheets[0].insertRule(
    '#covid-table tr[title=thursday] { background-color: hsl(220, 100%, 93%); }',
   );
   document.styleSheets[0].insertRule(
    '#covid-table tr[title=friday] { background-color: hsl(200, 100%, 95%); }',
   );
   document.styleSheets[0].insertRule(
    '#covid-table tr[title=saturday] { background-color: hsl(180, 100%, 97%); }',
   );
   ...
   saveButton.addEventListener('click', () => {
    const csv = `dates,${dataColumns.map(({ name }) => name).join(',')}\n${dates
     .map((date, i) => `"${date}",${dataColumns.map(({ data }) => data[i]).join(',')}`)
     .join('\n')}`;
