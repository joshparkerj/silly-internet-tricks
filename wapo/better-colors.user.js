...
(function betterColors() {
 ...
 for (let i = 0; i < styleSheets.length; i++) {
  ...
  const j = [...styleSheet.cssRules].findIndex(
   (rule) => rule.selectorText === '#pb-root' && rule.cssText.includes('background'),
  );
  if (j !== -1) {
 ...
 if (mainHeading) {
  ...
  topContentHeading.style.setProperty(
   'text-shadow',
   'rgba(127,204,102,.5) 10px 10px 10px, rgba(204,102,127,.5) -10px -10px 10px',
  );
