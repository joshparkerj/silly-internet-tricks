...
const getElementMatcher = function getElementMatcher(elementSelector) {
 ...
 if (id) {
  filters.push(
   (childNode) => childNode.attrs
    && childNode.attrs.find(({ name, value }) => name === 'id' && value === id[0].replace('#', '')),
  );
 }
 ...
 if (attributeSelectors) {
  filters.push((childNode) => attributeSelectors.every((attributeSelector) => {
   ...
   const attributeMatch = attributeContent.match(
    /^(?<attr>[\w-]+)\s*(?<operator>[~|^$*]?=)?\s*(?<attrValue>"[^"]*"|'[^']*'|[\w-]+)?$/,
   );
   ...
   if (operator && attrValue) {
    const attributeValues = childNode.attrs
      && childNode.attrs.filter(({ name }) => name === attr).map(({ value }) => value);
...
const querySelectorHelper = function querySelectorHelper(node, query) {
 ...
 for (let i = 0; i < selectorList.length; i++) {
  ...
  const { elementSelector } = elementSelectorAndRest.match(
   /(?<elementSelector>(\[[^\]]+\]?|[^\s+>~])+)/,
  ).groups;
