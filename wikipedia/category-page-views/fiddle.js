...
const getPageViewsThisMonth = function getPageViewsThisMonth() {
 ...
 const today = `${thisYear}${thisMonth.toString().padStart(2, '0')}${thisDay
  .toString()
  .padStart(2, '0')}`;
 ...
 const pageViewUrls = [...categoryLinks].map(
  (a) => `${apiUrl}${a.href.match(/[^/]*$/)[0]}/monthly/${lastMonthDate}/${today}`,
 );

 Promise.all(
  pageViewUrls.map((url) => fetch(url)
   .then((r) => r.json())
   .then((json) => json.items[0].views)),
 ).then((results) => results.forEach((views, i) => {
