...
(function randomPassword() {
  ...
  const wordsOfLength = (n) => (
    normals.filter(wordLengthIs(n)).concat(hards.filter(wordLengthIs(n)))
  );
  ...
  const randPass = Array(8).fill().map((e, i) => passComponents[i % passComponents.length]()).join('');
