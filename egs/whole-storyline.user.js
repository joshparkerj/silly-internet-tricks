...
(function userScript() {
  ...
  const getMore = (href, selector, inserter) => {
    fetch(href)
      ...
      .then((doc) => {
        const docStorylineValue = doc.querySelector("select[name='comic-storyline'] > option[selected]").value;
  ...
  showAllButton.innerText = `show all comics from storyline ${getStorylineName([...document.querySelectorAll("select[name='comic-storyline'] > option")])} ${storyline.innerText.trim()}`;
  showAllButton.addEventListener('click', () => {
    getPrev(prev.href);
    getNext(next.href);
  });

  document.querySelector('#leftarea').insertBefore(showAllButton, document.querySelector('div#chapter-dropdown + div + script + hr'));
}());
