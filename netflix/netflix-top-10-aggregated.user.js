...
(function userScript() {
 ...
 Promise.all(
  links.map(({ url, categoryName }) => fetch(dateMatch ? url.replace('.html', dateMatch[0]) : url)
   .then((r) => r.text())
  ...
  const tableHeaderCategorySelect = document.querySelectorAll(
   '#maincontent + div thead > tr > th',
  )[1];
  ...
  tableHeaderWeeks.insertAdjacentHTML(
   'beforebegin',
   '<th class="align-bottom border-bottom border-b-2 border-gray-800 pb-1 table-header-text"><span class="pageText">CATEGORY</span></th><th class="align-bottom border-bottom border-b-2 border-gray-800 pb-1 table-header-text"><span class="pageText">NAME</span></th>',
  );
 ...
 const currentWeek = new Date(
  document.querySelector('main #maincontent .select-week').textContent.split('-')[1].trim(),
 );
