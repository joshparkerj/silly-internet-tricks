...
(function svgElement() {
 ...
 const handler = (event) => {
  ...
  const newElement = parser
   .parseFromString(target.outerHTML, 'text/html')
   .querySelector('body > *');
  const setStyles = (n00b, old) => {
   const n00bComputedStyle = getComputedStyle(n00b);
   const oldComputedStyle = getComputedStyle(old);

   Object.keys(n00bComputedStyle)
    .filter((key) => n00bComputedStyle[key] !== oldComputedStyle[key])
    .forEach((key) => n00b.style.setProperty(key, oldComputedStyle[key]));
  };
