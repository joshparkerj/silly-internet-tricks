...
(function mostDownloadedUserScript() {
 const getMostDownloaded = () => {
  ...
  fetch('?activeTab=versions')
   ...
   .then((versions) => noDupes([...versions]).sort((a, b) => downloadNumber(b) - downloadNumber(a)))
   .then((sortedVersions) => sortedVersions.slice(0, 10))
   .then((mostDownloadedVersions) => {
    const versionLength = Math.max(...mostDownloadedVersions.map((v) => versionNumber(v).length));

    const downloadLength = Math.max(...mostDownloadedVersions.map((v) => downloadCount(v).length));

    const makeReadable = (v) => `version number ${versionNumber(v).padStart(versionLength, ' ')} : ${downloadCount(v).padStart(
     downloadLength,
     ' ',
    )} downloads`;
