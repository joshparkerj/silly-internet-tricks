...
`;
 ...
 const modifyMyCreations = () => {
  ...
  cards.forEach(async (card) => {
   ...
   const dataHref = window.location.href.replace(
    'my-creations',
    `_next/data/${buildId}/creation/${creationId}.json?cid=${creationId}`,
   );
...
`;
   ...
   if (algorithm === 'vqganclip') {
    ...
    appendText(
     creationSettings,
     `Overall Prompt Weight: ${percent(promptWeight)}.`,
     '[id^="creation-settings-overall-prompt-weight"',
    );
    if (initScale) {
     appendText(
      creationSettings,
      ` Start Image Weight:  ${percent(initScale)}.`,
      '[id^="creation-settings-overall-prompt-weight"',
     );
    }
    if (noiseInfluence) {
     appendText(
      creationSettings,
      ` Noise Weight: ${percent(noiseInfluence)}.`,
      '[id^="creation-settings-overall-prompt-weight"',
     );
    }
    ...
    appendText(
     creationSettings,
     (accuracyBoost && cutBatchesBoost && 'Extra') || (accuracyBoost && 'Standard') || 'None',
     '[id^="creation-settings-accuracy-boost"]',
    );
